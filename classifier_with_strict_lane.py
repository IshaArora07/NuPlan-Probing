#!/usr/bin/env python3
"""
Precompute EMoE scene labels + scene anchors from nuPlan (STRICT intersection traversal).
...
Key behavior (STRICT, as you requested):
  ✅ "at_intersection" classes ONLY if ego actually TRAVERSES the intersection:
        - either intersects INTERSECTION polygons along its trajectory, OR
        - actually traverses a LANE_CONNECTOR (map-matched along ego path)
  ✅ LEFT/RIGHT/STRAIGHT at intersection decided by NET heading (geometry),
     and (optionally) verified by the connector turn_type of the traversed connector(s).
  ✅ Avoid false positives:
        - motion gate
        - minimum travel distance for turn decisions
        - connector traversal requires consistent map-match hits (not just “nearby vote”)

Notes:
  - This script is CPU / I/O / GeoPandas-Shapely bound. GPU won’t help meaningfully.
  - Speed comes from: spatial indexes + caching per map + avoiding iterrows.

Run:
  export NUPLAN_DATA_ROOT=/path/to/nuplan
  export NUPLAN_MAPS_ROOT=/path/to/nuplan/maps

  python precompute_emoe_labels_anchors_strict.py \
    --split mini \
...
def stage1_from_tags_and_type(scenario_type: str, tags: List[str]) -> Tuple[Optional[int], Optional[str]]:
    ...
    Requirements:
      ...
      - intersection tags do NOT decide direction here; strict traversal + geometry decides later
    """
...
# --------------------------------------------------------------------------------------
# Map layer cache (per map_api instance)  ### NEW
# --------------------------------------------------------------------------------------
class MapLayerCache:
    """
    Cache vector layers + spatial indexes + prepared geometries for faster per-scenario queries.
    """
    def __init__(self):
    ...
    def get(self, map_api) -> Dict[str, Any]:
        ...
        # Intersection prepared geometries (faster contains)
        inter_prepared = []
        inter_geoms = []
        inter_sindex = None
        if inter_gdf is not None and len(inter_gdf) > 0:
            inter_sindex = self._safe_sindex(inter_gdf)
            # keep raw geometry list
            inter_geoms = list(inter_gdf.geometry.values)
            # prepared geometries help for repeated contains() calls
            inter_prepared = [prep(g) for g in inter_geoms if g is not None]

        # Connector geometry + turn_type column + sindex
        conn_sindex = None
        conn_geoms = []
        conn_turn_vals = None
        conn_turn_col = ""
        if conn_gdf is not None and len(conn_gdf) > 0:
            conn_sindex = self._safe_sindex(conn_gdf)
            conn_geoms = list(conn_gdf.geometry.values)

            # detect turn type column
            ...
            if conn_turn_col:
                conn_turn_vals = np.asarray(conn_gdf[conn_turn_col].values)
            else:
                conn_turn_vals = None
...
# --------------------------------------------------------------------------------------
# STRICT intersection traversal from INTERSECTION polygons  ### CHANGED
# --------------------------------------------------------------------------------------
def intersection_traversal_from_layer(
    ...
    at least `min_hits` times (to avoid single noisy touch).
    """
    inter_gdf = cache.get("inter_gdf", None)
    if inter_gdf is None or len(inter_gdf) == 0:
        return False, float("inf"), 0

    inter_sindex = cache.get("inter_sindex", None)
    inter_geoms = cache.get("inter_geoms", [])
    # prepared list only helps for contains() when we already have candidate geom; still ok
    # (we don't map prepared->original index here; we just use raw geometries)

    tol = float(intersection_tol_m)
    min_dist = float("inf")
    hits = 0

    # iterate sampled points
    for i in range(0, len(xs), max(1, sample_step)):
        p = Point(float(xs[i]), float(ys[i]))

        # candidate polygons by bbox query
        ...
        if hits >= int(min_hits) and min_dist <= tol:
            # early exit once traversal is confidently established
...
# --------------------------------------------------------------------------------------
# STRICT connector traversal by ego map-matching to connectors  ### NEW / CHANGED
# --------------------------------------------------------------------------------------
def _turn_type_to_label(v) -> str:
    """
    nuPlan LaneConnectorType enum in maps_datatypes.py:
...
def connector_traversal_mapmatch(
    ...
    """
    Map-match ego sampled points to LANE_CONNECTOR geometries and detect *traversal*.

    Returns:
      {
        "traversed_connector": bool,
        "hits": int,
        "matched_samples": int,
        "turn_counts": {...},
        "best_type": str,
    ...
    Strictness:
      - A point "matches" a connector only if distance <= match_radius_m
      - Optionally gates by heading consistency between ego heading and connector local direction
        (approx via nearest segment direction) – we implement a lightweight version:
        if heading_gate_deg <= 0 -> disabled
    """
    conn_gdf = cache.get("conn_gdf", None)
    ...
    # helper to estimate connector direction near point (very lightweight):
    def approx_geom_heading(geom, p: Point) -> Optional[float]:
        """
        Approximate direction by taking the nearest point on the line and a tiny step along it.
        Works for LineString; returns radians in [-pi, pi].
        If this fails, returns None.
        """
        try:
            # shapely ops: project -> interpolate
            s = geom.project(p)
            # take two close points along the line to estimate tangent
    ...
    for i in range(0, len(xs), max(1, sample_step)):
        ...
        # vote turn type for this matched connector
...
# --------------------------------------------------------------------------------------
# Stage 2+3: STRICT intersection context + geometry decision  ### CHANGED
# --------------------------------------------------------------------------------------
def classify_strict_intersection_logic(
    ...
    *,
    ...
    """
    STRICT logic:
      - Intersection context is TRUE only if ego actually traverses:
           (A) intersection polygons OR (B) a lane connector (map-matched).
      - LEFT/RIGHT/STRAIGHT at intersection decided by NET heading.
      - Connector type used only as verification (optional).

    Prevents:
      - "intersection present nearby" -> at_intersection label
      - "connector near ego" -> turn label
    """
    T = len(xs)
    if T < 3:
    ...
    # motion gate (stationary can't be turn)
    ...
    # tag hint exists, but does NOT force intersection class (traversal must be true)
    ...
    # STRICT connector traversal by map-match
    ...
    if traversed_intersection:
        ...
        if abs_dh <= NET_STRAIGHT_MAX_AT_INTERSECTION:
            # (optional) if connector strongly indicates turn, we can keep straight but flag it
        ...
        if NET_TURN_MIN_AT_INTERSECTION <= abs_dh <= NET_TURN_MAX_AT_INTERSECTION:
            ...
            # connector verification (only if ego traversed connector and it is confident)
            if traversed_connector:
                bt = str(conn_mm.get("best_type", "NONE"))
                br = float(conn_mm.get("best_ratio", 0.0))
                if br >= float(connector_verify_min_ratio) and bt in ("LEFT", "RIGHT", "STRAIGHT", "UTURN"):
                    debug["connector_verify_note"] = f"best={bt}, ratio={br:.2f}"
                    # if connector strongly contradicts geometry, keep geometry but flag it
    ...
    # -------------------------
    # STRICT non-intersection branch
    # -------------------------
...
# --------------------------------------------------------------------------------------
# Combined classification pipeline (keeps your stage logic)
# --------------------------------------------------------------------------------------
def classify_emoe_for_scenario(
    ...
    *,
    ...
    Stage 2+3: STRICT traversal + geometry
    """
...
def main():
    ...
    print(f"[INFO] connector_match_radius_m={args.connector_match_radius_m}, min_hits={args.connector_min_hits}, step={args.connector_sample_step}, heading_gate={args.connector_heading_gate_deg}")
    print(f"[INFO] NET_TURN_MIN_AT_INTERSECTION(deg)={math.degrees(NET_TURN_MIN_AT_INTERSECTION):.1f}")
    ...
    # hard cap for correct tqdm denominator
        ...
        for scenario in tqdm(scenarios, total=len(scenarios), desc="Classifying + collecting endpoints"):
            ...
            record = {
                ...
                "debug": {
                    ...
                    "connector_verify_note": debug.get("connector_verify_note", None),
                    "connector_verify_conflict": debug.get("connector_verify_conflict", None),
