...
class PlutoFeatureBuilder(AbstractFeatureBuilder):
    def __init__(
        ...
        self.inference: Optional[bool] = None
        ...
        # ------------------------------------------------------------------
        # EMoE: scene labels (for router supervision)
        # ------------------------------------------------------------------
        # Hardcode the path here; you just replace the string.
        # File format: JSON lines with keys:
        #   "token": str
        #   "emoe_class_id": int
        #   "emoe_class_name": str
        #   "travel_distance": float (unused here)
        self._emoe_label_path: Optional[str] = (
            "/ABSOLUTE/PATH/TO/scene_labels.jsonl"  # TODO: replace this
        )
        self._emoe_label_map: Optional[Dict[str, Dict[str, Any]]] = None

    # ----------------------------------------------------------------------
    # EMoE label loading helpers
    # ----------------------------------------------------------------------

    def _load_emoe_label_map(self) -> None:
        """Load scene_labels.jsonl once into memory."""
        if self._emoe_label_map is not None:
            return

        self._emoe_label_map = {}

        if self._emoe_label_path is None:
            return

        try:
            with open(self._emoe_label_path, "r") as f:
                for line in f:
                    line = line.strip()
                    if not line:
                        continue
                    ...
                    token = rec.get("token")
                    class_id = rec.get("emoe_class_id")
                    class_name = rec.get("emoe_class_name")

                    if token is None or class_id is None:
                        continue

                    self._emoe_label_map[str(token)] = {
            ...
            warnings.warn(
                f"[PlutoFeatureBuilder] scene_labels file not found at "
                f"{self._emoe_label_path}. EMoE router will train without labels.",
                RuntimeWarning,
            )
            self._emoe_label_map = {}

    def _attach_emoe_label(
        self, data: Dict[str, Any], scenario_token: Optional[str]
    ) -> None:
        """
        Attach EMoE scene label to data["emoe"] if we have a label for this token.
        """
        if scenario_token is None or self._emoe_label_path is None:
            return

        self._load_emoe_label_map()
        if not self._emoe_label_map:
            return

        info = self._emoe_label_map.get(str(scenario_token))
        if info is None:
            return
    ...
    def get_features_from_scenario(
        ...
        """
        Called during offline training/validation data generation.
        Here we have access to the scenario, so we can attach EMoE labels.
        """
        ...
        # scenario token for EMoE labels (depends on how you built the JSONL)
    ...
    def get_features_from_simulation(
        ...
        """
        Called during closed-loop simulation.
        No scenario token here, so we don't attach EMoE labels.
        """
        ...
        return self._build_feature(
            ...
            inference=True,
            scenario_token=None,  # no labels during simulation
        )
    ...
    def _build_feature(
        ...
        """
        Core feature construction used by both training and simulation.
        """
        if present_idx < 0:
        ...
        traffic_light_status = list(traffic_light_status)  # note: tl is an iterator
        ...
        # ------------------------------------------------------------------
        # EMoE: attach scene label if available (training / labelled val only)
        # ------------------------------------------------------------------
        if not inference:
            self._attach_emoe_label(data, scenario_token)
    ...
    def _get_ego_features(self, ego_states: List[EgoState]):
        """Rear axle velocity and acceleration are in ego frame; rotate to local."""
        T = len(ego_states)
    ...
    def _get_map_features(
        ...
        for lane in lane_objects:
            ...
            polygon_speed_limit[idx] = lane.speed_limit_mps if speed_limit else 0
