...
class PlutoFeature(AbstractModelFeature):
    ...
    def collate(cls, feature_list: List[PlutoFeature]) -> PlutoFeature:
        ...
        has_emoe = "emoe" in feature_list[0].data
        ...
        if feature_list[0].data_n is not None:
            ...
            if "emoe" in feature_list[0].data and "emoe_class_id" in feature_list[0].data["emoe"]:
              batch_data["emoe"] = {
                "emoe_class_id": torch.as_tensor(
                  [_scalar_int(f.data["emoe"]["emoe_class_id"]) for f in feature_list]
                  + [_scalar_int(f.data_p["emoe"]["emoe_class_id"]) for f in feature_list]
                  + [_scalar_int(f.data_n["emoe"]["emoe_class_id"]) for f in feature_list],
                  dtype = torch.long
                )
              }
