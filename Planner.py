...
class PlutoPlanner(AbstractPlanner):
    ...
    def __init__(
        ...
        :param planner: TorchModuleWrapper model used for inference.
        """
        self._render = render
        self._imgs = []
        ...
        self._scenario_type = scenario.scenario_type if scenario is not None else None
    ...
    def _infer_model(self, features: FeaturesType) -> npt.NDArray[np.float32]:
        ...
        NOTE: this method is not used in your current _run_planning_once; kept for completeness.
        """
        action = self._model_loader.infer(features)  # type: ignore[attr-defined]
    ...
    def compute_planner_trajectory(
        ...
        # (feature building timing is tracked elsewhere in your original; keep minimal)
    ...
    def _run_planning_once(self, current_input: PlannerInput):
        """
        Main planning loop. This is where we adapt EMoE outputs to what PlutoPlanner expects.
        """
        ego_state = current_input.history.ego_states[-1]
        ...
        # Some wrappers return (out, ...) -> normalize
        if isinstance(out, tuple):
            out = out[0]

        # ------------------------------------------------------------
        # EMoE COMPATIBILITY LAYER (NO RETRAINING)
        # ------------------------------------------------------------
        # Original PLUTO expects:
        #   out["candidate_trajectories"][0] -> numpy float64
        #   out["probability"][0] -> numpy
        # Your EMoE model returns:
        #   out["trajectory"]   : [B, 1, Ka, T, 6]
        #   out["probability"] : [B, 1, Ka]
        #   out["prediction"]  : ...
        # ------------------------------------------------------------
        ...
        if "candidate_trajectories" in out:
            candidate_trajectories = (
                out["candidate_trajectories"][0].detach().cpu().numpy().astype(np.float64)
            )
        else:
            traj = out["trajectory"]
            if not (isinstance(traj, torch.Tensor) and traj.ndim == 5):
                raise KeyError(
                    "Missing 'candidate_trajectories' and 'trajectory' is not a tensor with shape [B,1,Ka,T,6]. "
                    f"type={type(traj)} shape={getattr(traj, 'shape', None)} keys={list(out.keys())}"
                )
            # [Ka, T, 6]
            traj_kt6 = traj[0, 0]
            # Assumption: first 3 dims are (x, y, heading) in LOCAL frame.
            candidate_trajectories = traj_kt6[..., :3].detach().cpu().numpy().astype(np.float64)

        # Probability
        if "probability" in out:
            prob = out["probability"]
            if isinstance(prob, torch.Tensor) and prob.ndim == 3:
                probability = prob[0, 0].detach().cpu().numpy()
            elif isinstance(prob, torch.Tensor) and prob.ndim == 2:
                probability = prob[0].detach().cpu().numpy()
            else:
                # fallback
                probability = out["probability"][0].detach().cpu().numpy()
        else:
            probability = np.ones((candidate_trajectories.shape[0],), dtype=np.float32) / max(
                1, candidate_trajectories.shape[0]
            )

        # Predictions (key mapping)
        if self._use_prediction:
            if "output_prediction" in out:
                ...
                predictions = out["prediction"][0].detach().cpu().numpy()
            else:
                predictions = None
        else:
            predictions = None

        # Ref-free trajectory (unchanged)
        ref_free_trajectory = (
            (out["output_ref_free_trajectory"][0].detach().cpu().numpy().astype(np.float64))
            if "output_ref_free_trajectory" in out
            else None
        )

        # Trim candidates, convert to global in _trim_candidates
    ...
    def _trim_candidates(
        ...
        """
        candidate_trajectories: (n_ref, n_mode, 80, 3) OR (n_mode, 80, 3)
        probability: (n_ref, n_mode) OR (n_mode,)
        """
    ...
    def _get_agent_info(self, data, predictions, ego_state: EgoState):
        """
        predictions: (n_agent, 80, 2 or 3 or 5)
        """
    ...
    def generate_planner_report(self, clear_stats: bool = True) -> PlannerReport:
        ...
        report = MLPlannerReport(
            compute_trajectory_runtimes=self._compute_trajectory_runtimes,  # type: ignore[attr-defined]
            feature_building_runtimes=self._feature_building_runtimes,
            inference_runtimes=self._inference_runtimes,
        )
        if clear_stats:
            self._compute_trajectory_runtimes: List[float] = []  # type: ignore[attr-defined]
