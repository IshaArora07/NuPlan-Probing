...
class PlanningModel(TorchModuleWrapper):
    def __init__(
        ...
        self.use_hidden_proj = use_hidden_proj  # FIX: store the flag
        ...
        # Load anchors from file if provided, else zeros
        if anchors_path is not None:
            import numpy as np
            anchors_np = np.load(anchors_path)
            assert anchors_np.shape == (num_scene_types, num_modes, 2), (
                f"Expected anchors shape ({num_scene_types}, {num_modes}, 2), "
                f"got {anchors_np.shape}"
            )
            anchors_xy_init = torch.from_numpy(anchors_np).float()
        else:
    ...
    def forward(
        ...
        """
        Args:
            data:             standard feature dict
            gt_scene_labels:  [B] long tensor of ground truth scene class ids.
                              Only used when force_routing=True.
            force_routing:    If True and gt_scene_labels is provided, use GT
                              labels for query generation and expert selection
                              instead of the router's predicted scene_idx.
                              The router still runs and returns logits for the
                              CE loss — only the routing decision is overridden.
        """
        ...
        # ---------------- Router (always runs for loss) ----------------
        scene_logits, scene_idx_pred = self.scene_router(scene_tokens=x)

        # ---------------- Routing decision ----------------
        # If teacher forcing is active and GT labels are available,
        # override the routing decision with GT labels.
        # The router logits are still returned for the CE supervision loss.
        if force_routing and gt_scene_labels is not None:
            routing_idx = gt_scene_labels
        else:
            routing_idx = scene_idx_pred

        # ---------------- Query generation ----------------
        mode_queries = self.mode_query_generator(scene_labels=routing_idx)

        # ---------------- Planning decoder ----------------
        decoded_queries, traj_modes, scores_modes = self.planning_decoder(
            mode_queries=mode_queries,
            scene_tokens=x,
            scene_idx=routing_idx,
            key_padding_mask=key_padding_mask,
        )

        trajectory = traj_modes.unsqueeze(1)      # [B, 1, Ka, T, 6]
        probability = scores_modes.unsqueeze(1)   # [B, 1, Ka]
        ...
        out = {
            ...
            "router_idx": scene_idx_pred,   # always the predicted idx for logging
            "routing_idx": routing_idx,      # what was actually used for routing
