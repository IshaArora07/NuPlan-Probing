...
with an added lane-following alignment check to avoid mislabeling curved lane-following
as "others".
...
Key behavior:
  ...
  ✅ NEW: lane-following alignment (instantaneous ego heading vs lane/connector tangent)
...
# ### NEW: mild-curvature guard bands (used with lane-following alignment)
...
class MapLayerCache:
    ...
    def get(self, map_api) -> Dict[str, Any]:
        ...
        # ### NEW: also cache LANE layer for lane-following outside intersections
        lane_gdf = self._get_vector_map_layer(map_api, SemanticMapLayer.LANE)

        # Intersection prepared geometries
        inter_geoms = []
        inter_sindex = None
        ...
        # ### NEW: Lane geometry + sindex
        lane_sindex = None
        lane_geoms = []
        if lane_gdf is not None and len(lane_gdf) > 0:
            lane_sindex = self._safe_sindex(lane_gdf)
            lane_geoms = list(lane_gdf.geometry.values)
...
# --------------------------------------------------------------------------------------
# ### NEW: lane-following alignment check
# --------------------------------------------------------------------------------------
def lane_following_alignment(
    ...
    *,
    ...
    """
    Compute how well ego heading follows the tangent of the lane/lane_connector geometry.

    We map-match each sampled ego point to the nearest lane (or connector) geometry within radius,
    estimate tangent heading there, and compute heading error |wrap(lane_h - ego_h)| in degrees.
    ...
    if not geoms:
        return {"layer": layer_name, "hits": 0, "samples": 0, "hit_ratio": 0.0,
                "median_err_deg": float("inf"), "mean_err_deg": float("inf")}
    ...
    for i in range(0, len(xs), max(1, sample_step)):
        ...
        for j in cand_idx:
            ...
            if d <= best_d:
                ...
                if gh is None:
                    continue
                # optional heading gate (reject wildly inconsistent tangent)
                if heading_gate > 0:
    ...
    if hits < int(min_hits) or len(errs) == 0:
        return {"layer": layer_name, "hits": int(hits), "samples": int(samples), "hit_ratio": hits / max(1, samples),
                "median_err_deg": float("inf"), "mean_err_deg": float("inf")}
...
# --------------------------------------------------------------------------------------
# Stage 2+3: STRICT intersection context + geometry decision
# --------------------------------------------------------------------------------------
def classify_strict_intersection_logic(
    ...
    *,
    ...
    STRICT logic (unchanged core):
      ...
      - Connector type used only as verification.

    ### NEW:
      - lane-following alignment:
          * intersection "gap" (straight_max < abs_dh < turn_min) can become STRAIGHT_AT_INTERSECTION
          * non-intersection curved lane-following can become STRAIGHT_NON_INTERSECTION even if total_abs is large
    """
    T = len(xs)
    if T < 3:
    ...
    # ### NEW: lane-following alignment
    # - if in intersection context, align to connectors (because direction changes happen on connectors)
    # - otherwise align to lanes
    ...
    lane_following_ok = (np.isfinite(lf.get("median_err_deg", float("inf")))
    ...
    if traversed_intersection:
        ...
        if dist < MIN_DIST_TURN_AT_INTERSECTION:
            ...
            # ### CHANGED: if lane-following is very strong, still treat as straight
            if lane_following_ok:
                return 1, "stage3_intersection_lane_following_motion_guard", debug
        ...
        # ### NEW: intersection "gap" fix
        # If it's not straight enough (<12°) but also not a turn (>=35°),
        # then it is often "curved straight" while staying in-lane through the intersection.
    ...
    # ### NEW: curved lane-following outside intersection
    # If net heading is still modest and we are closely aligned to lane tangent,
    # treat as straight_non_intersection even if total_abs is large.
...
def classify_emoe_for_scenario(
    ...
    *,
    ...
    Stage 2+3: STRICT traversal + geometry + lane-following fixes
    """
...
def main():
    ...
    print(f"[INFO] NET_STRAIGHT_MAX_AT_INTERSECTION(deg)={math.degrees(NET_STRAIGHT_MAX_AT_INTERSECTION):.1f}")
